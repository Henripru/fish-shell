...
void assert_is_main_thread(const char *who) {
    if (!is_main_thread() && !thread_asserts_cfg_for_testing) {
        debug(1, "%s called off of main thread.", who);
        debug(1, "Break on debug_thread_error to debug.");
        debug_thread_error();
    }
}
...
void assert_is_not_forked_child(const char *who) {
    if (is_forked_child()) {
        debug(1, "%s called in a forked child.", who);
        debug(1, "Break on debug_thread_error to debug.");
        debug_thread_error();
    }
}
...
void assert_is_background_thread(const char *who) {
    if (is_main_thread() && !thread_asserts_cfg_for_testing) {
        debug(1, "%s called on the main thread (may block!).", who);
        debug(1, "Break on debug_thread_error to debug.");
        debug_thread_error();
    }
}
...
void assert_is_locked(void *vmutex, const char *who, const char *caller) {
    ...
    if (0 == pthread_mutex_trylock(mutex)) {
        debug(1, "%s is not locked when it should be in '%s'", who, caller);
        debug(1, "Break on debug_thread_error to debug.");
        debug_thread_error();
